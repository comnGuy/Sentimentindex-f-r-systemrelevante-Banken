...
\label{Woerterbuch}
<<<<<<< HEAD
Ein Mensch ist in der Lage nachdem Lesen eines Textes zu beurteilen welche Stimmung dieser wiederspiegelt. Das Tweetbeispiel: "Der Kurs ist gut! #bitcoin", spiegelt eine positive Resonanz bezÃ¼glich Bitcoin wieder. Eine Variante, wie mittels Text Mining ein Sentiment gebildert werden kann, ist die einzelnen WÃ¶rter oder Wortgruppen (n-gramms) mittels einem WÃ¶rterbuch zu bewerten. Wird bezÃ¼glich unseren Beispiel das Wort gut betrachtet, wird dieser wahrscheinlich positiv bewertet.
=======
...
>>>>>>> 7b2502ece3bc74c7844471a6f5e58d8deada068c
...
<<<<<<< HEAD
Ein \textbf{Hononym} ist der Begriff fÃ¼r die Doppeldeutigkeit eines Wortes, zum Beispiel \textit{Bank}. Wird ein Wort fÃ¼r ein WÃ¶rterbuch bewertet, muss der Kontext in dem sich das WÃ¶rterbuch befindet, beachtet werden. Angenommen es existieren zwei WÃ¶rterbÃ¼cher, wobei jeweils eins fÃ¼r die Natur und eins fÃ¼r den Immobiliensektor abgestimmt ist. Das Wort \textit{Bank} kann in diesem Fall zwei unterschiedliche Bewertung pro WÃ¶rterbuch erhalten. In der Natur kÃ¶nnte eine Bank eher als positiv bewertet werden, wohingegen die Bank fÃ¼r den Immobiliensektor eher negativ ausgelegt werden kÃ¶nnte.

Um die \textbf{GÃ¼te eines WÃ¶rterbuches} beurteilen zu kÃ¶nnen, werden Tweets benÃ¶tigt, die in irgendeiner Art und Weise durch eine Bewertung markiert sind. Der gegebene Datensatz aus Kapitel \ref{sub:scanned_data} von Tweets erhielt kein Merkmal, welches auf eine Stimmung hinweisen kÃ¶nnte. Die Konsequenz war Teile des Datensatzes manuell und subjektiv zu bewerten. In Abbilung \ref{fig:testdata_bewertet} sind die Bewertungungen mit \textit{sentimentOriginal}, den Tweettext mit \textit{full\_text} und die Nummerierung mit \textit{X} zu sehen.
=======
...
<<<<<<< HEAD
In Tabelle \ref{tab:used_wordbooks} sind die verwendeten WÃ¶rterbÃ¼cher mit deren Bewertungsart, ob eine Intelligenz verfÃ¼gbar und in welcher Programmiersprache dieses WÃ¶rterbuch ursprÃ¼nglich vorhanden war, aufgelistet. Die WÃ¶rterbÃ¼cher "bing" und "loughran" arbeiten mit positiven und negativen Bewertungen, "afinn" arbeitet mit einer Bewertungsreichweite von -5 bis 5, wobei -5 die negativste- und 5 die positivste Stimmung wiederspiegelt. Das WÃ¶rterbuch "vaderSentiment" wurde von 10 unabhÃ¤ngigen Menschen manuell mit der Reichweite -4 bis 4 bewertet und davon der Durchschnitt und die Standardabweichung berechnet. Weiterhin wurde eine Logik von den Machern hinterlegt, die Verneinung, Satzzeiten, groÃŸ geschriebene WÃ¶rter u. v. m. bei der Berechnung des Sentiments beachtet. Die Logik wurde in Python umgesetzt.
=======
...
<<<<<<< HEAD
 * Die \textit{negativen} Treffer sind ausschlieÃŸlich die Anzahl der negativen Treffer.
 * Die \textit{positiven} Treffer sind ausschlieÃŸlich die Anzahl der positiven Treffer.
 * Die \textit{neutralen} Treffer sind ausschlieÃŸlich die Anzahl der neutralen Treffer.
=======
...
<<<<<<< HEAD
Um das Stemming durchzufÃ¼hren, wurde das R-Package \textit{corpus} verwendet <!-- Verlinken TODO -->. Die folgenden Varianten des Stemmings wurden durchgefÃ¼hrt. textit{corpus} unterstÃ¼tzt vom Haus aus 17 unterschiedliche Sprachen, um ein Wortstamm zu bilden. Da der gegebene Datensatz aus englischen Texten besteht, wurde nur die englische Sprache des Standard-Packetes betrachtet. Die folgenden Stemmer und Rechtschreibkorrekturen wurden betrachtet.

 * Der \textit{Hunspell stemmer} soll prezisere StÃ¤mme bilden.
 * Der \textit{Dictionary stemmer} wird on-the-fly von lexoconista.com als Tupels von (WÃ¶rter, Stamm) geladen und mittels einer Funtkion den corpus-Package Ã¼bergeben.
 * Hunspell liefert eine \textit{Rechtschreibkorrektur} mit. Zu Beginn wird geprÃ¼ft, ob das Wort im Hunspell-Stemming-WÃ¶rterbuch existiert. Falls das Wort existiert, wird der Wortstamm zurÃ¼ck gegeben. Falls das Wort im Hunspell-Stemming-WÃ¶rterbuch nicht existiert, werden versucht Ã¤hnliche WÃ¶rter mittels der bereits existierten Funktion zu finden.
 
FÃ¼r die gestemmten Tweets wurden die gleichen Auswertungen berechnet, wie in Abbildung \ref{fig:ubereinstimmung_nothing} zu sehen ist. Dabei verschlechterten alle Stemming-Verfahren das Ergebnis. Die einzige Ausnahme gilt der Rechtschreibkorrektur, die insgesamt zwei Tweets mehr erkennt.
...
FÃ¼r die Berechnung der Sentimentindizes wird nach dem Buch \textit{Text Mining with R} von \textit{Julia Silge and David Robinson} vorgegangen. <!-- TODO Referenz? --> Im Buch werden die Sentimentindizes mittels dem Prinzip \textit{tidy data} in R erstellt. Ein Beispiel fÃ¼r einen einfachen Sentimentindex, der positive und negative WÃ¶rter fÃ¼r jede Woche zÃ¤hlt, soll vorgestellt werden. Zur Berechnung des Sentimenindex wird das vorbereitete Dataframe aus dem Kapitel \textit{Datenaufbereitung \ref{sub:datenaufbereitung}} benutzt. Zuerst wird der vorbereitete Dataframe mit dem WÃ¶rterbuch \textit{Bing} gejoint <!-- Vielleicht join nochmal beschreiben oder eindeutiger hinschreiben --> mit der gleichnamige Spalte \textit{word}. Im Anschluss wird das Dataframe nach Wochen gruppiert und dabei werden die verschiedene AusprÃ¤gungen (\textit{positiv} und \textit{negativ}) in der Spalte \textit{Sentiment} gezÃ¤hlt. FÃ¼r die gezÃ¤hlten AusprÃ¤gungen pro Woche entsteht eine neue Spalte \textit{n}. In der Spalte \textit{Sentiment} stehen die WÃ¶rter (AusprÃ¤gungen) \textit{positiv} oder \textit{negativ} passend zu dem Wort in der Spalte \textit{word} im Datensatz. Um einen Index pro Woche zu berechnen, wird eine Spalte fÃ¼r jede AusprÃ¤gung mit ihrer Anzahl pro Woche aus den Spalten \textit{Sentiment} und $n$ erzeugt. Zum Schluss wird eine neue Spalte \textit{Sentiment} gebildet, indem die Anzahl der positiven minus den negativen WÃ¶rter stehen ($Sentiment=positiv-negativ$). Der Sentimentindex \textit{Sentiment} wird als Barplot dargestellt.
=======
...
```
 ...
 \end{figure}
 <!-- Ist das nicht der wÃ¶chentliche Mittelwert? TODO -->
...
In diesem Abschnitt werden Methoden angerissen, die die MÃ¶glichkeit bieten, Tweets und Sentimentindizes zu analysieren. Um einen ersten Eindruck von den Tweets zu erhalten, ist es vom Vorteil die am hÃ¤ufigsten vorkommenden positivsten und negativsten WÃ¶rter zu visualisieren. Die Visualisierung wird mittels einem Balkendiagramm umgesetzt.
...
\end{figure}
<!-- Den obigen Teil kÃ¶nnte man auch weglassen, oder? TODO -->
```{r eval=FALSE, echo=FALSE}
