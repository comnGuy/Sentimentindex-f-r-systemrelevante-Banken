...
>>>>>>> 54ea087412e6a039fead8af94d4cf7075c003d12
...
#Aufbereitung der Daten----------------------------------
 file<-"~/textmining/R-projekt/BeckerSeminar2/Testing/Daten2012usa.csv"
...
```
 \begin{figure}[H]
  ...
  \includegraphics[width=1\textwidth]{Pictures/sentiment_bing_week.png}
   	\caption{Sentimentindex: wöchentliche Differenz zwischen positive und negative Wörter }
 \end{figure}

Es wurde eine viel Zahl von Sentimentindizes berechnet mit den Wörterbüchern, die im Kapitel \textit{Wörterbücher} vorgestellt wurden. In der unteren Liste werden einige Sentimentindizes mit ihren Funktionen aufgeführt, die von uns implementiert wurden in R. Die Funktionen der aufgezählten Sentimentindizes werden in der R-Datei \textit{SentimentFunctionChris.R} zu bereitgestellt. Bei allen Funktionen müssen zwei Parameter übergeben werden, den vorbereitete Dataframe und einen String (\textit{Woche} und \textit{Monat}) der angbit, ob ein Sentiment pro Woche oder pro Monat berechnet wird. 
\begin{itemize}
\item \textit{Wörterbuch Bing:} Mit der Funktion \textit{Sentiment$\_$bing$\_$postive$\_$minus$\_$negative$\_$socre$\_$means} können positive und negative Wörter gezählt werden pro Woche oder pro Monat. Im Anschluss wird die Differenz zwischen der Anzahl positiv und den negative Wörter genommen und durch die Gesamtanzahl an positiven und negativen Wörter geteilt. 
