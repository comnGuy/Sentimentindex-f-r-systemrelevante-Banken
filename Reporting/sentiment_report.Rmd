...
<!--- Beginn --->
\section{Ergebnisse} 
Das Ziel dieser Ausarbeitung ist es einen Zusammenhang zwischen der Stimmung (\textit{Sentiment}) in den Banke-Tweets aus $2012$ und Aktienindizes nachzuweisen. Es besteht die Annahme, dass Auf- und Abwärtsbewegungen im Aktienindex einen Einfluss auf die Stimmung in den Tweets besitzen. Eine weitere Annahme wäre, dass die selben wirtschaftlichen Ereignissen sich gleicherweise auf die Banken-Tweets und den Aktienindizes auswirken und somit die Stimmung in den Tweets ein gewisse Voraussetzung besitzen um Aktienindizes vorhersagen zu können. Mittels erzeugten Sentimentindizies aus den Tweets können Zusammenhänge sichtbar gemacht und Regressionen auf Aktienindizes durchgeführt werden.
 \begin{figure}[H]
   	\centering
...
\item \textbf{Step 2:} Um mit den Daten besser arbeiten zu können, müssen die Tweets mit einer Spalte \textit{Month2} und \textit{week} ergänzt werden. Mit den Spalten \textit{Month2} und \textit{week} können Indizies für Monaten und Wochen berechnet werden, dafür wird die Funktion \textit{Kalenderwochen} verwendet.
\item \textbf{Step 3:} Im Anschluss werden die Tweets, die mehrfach im Dataframe vorkommen, mit der Funktion \textit{Distinct} herausgefiltert.
