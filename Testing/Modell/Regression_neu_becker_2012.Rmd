---
title: "Bernhard Preisler und Christian Hofmann"
output: pdf_document
---
...
#Aufbereitung der Daten------------------------------------------------------------------
 ...
 daten_usa<-Datei_einlesen("Daten2012usa.csv")
 daten_europa<-Datei_einlesen("Daten2012europaneu3.csv")
...
dow_jones <- read_excel("Daten/Dow_jones_rendite_2012.xlsx")
...
library(readr)
daten_usa_new_sentiment_2_vader<-read_delim("Daten/daten_usa_new_sentiment _vader.csv",";", escape_double = FALSE, trim_ws = TRUE)
...
#---------------------------------------------------------------------------------
reg_vorbereitung_vader<-generateLinearRegressionDataFrame(reg_vader_group_wochen,prediction = 1, columnSizeSentiment = 3, sentimentColumnName = 'score', renditeColumnName = 'Veraenderung', renditeColumnNameOutput = 'Rendite')
colnames(reg_vorbereitung_wader)<-c("Sentiment1","Sentiment2", "Sentiment3", "Rendite")
regression_vader<-lm(Rendite~ Sentiment1 + Sentiment2+ Sentiment3,data=reg_vorbereitung_vader)
summary(regression_vader)
coeftest(regression_wader, df = Inf, vcov = NeweyWest)

#------------------------------------------
reg_vorbereitung_vader<-generateLinearRegressionDataFrame(reg_vader_group_wochen,prediction = 1, columnSizeSentiment = 6, sentimentColumnName = 'score', renditeColumnName = 'Veraenderung', renditeColumnNameOutput = 'Rendite')

colnames(reg_vorbereitung_vader)<-c("Sentiment1","Sentiment2", "Sentiment3","Sentiment4","Sentiment5","Sentiment6", "Rendite")
regression_vader<-lm(Rendite~ Sentiment1+Sentiment2+Sentiment3+Sentiment4+Sentiment5+Sentiment6,data=reg_vorbereitung_vader)
summary(regression_vader)
coeftest(regression_vader, df = Inf, vcov = NeweyWest)
data.new<-data.frame(reg_vorbereitung_vader$Sentiment1,reg_vorbereitung_vader$Sentiment2,reg_vorbereitung_vader$Sentiment3,reg_vorbereitung_vader$Sentiment4,reg_vorbereitung_vader$Sentiment5,reg_vorbereitung_vader$Sentiment6)
colnames(data.new)<-c("Sentiment1","Sentiment2", "Sentiment3","Sentiment4","Sentiment5","Sentiment6")
y=predict(regression_vader,data.new)
x<-seq(1,46,by=1)
plot(dow_jones$week,dow_jones$Veraenderung)
lines(x,y,type="l")
#---------------------------------------------------------------------------
reg_vorbereitung_vader<-generateLinearRegressionDataFrame(reg_vader_group_wochen,prediction = 1, columnSizeSentiment = 9, sentimentColumnName = 'score', renditeColumnName = 'Veraenderung', renditeColumnNameOutput = 'Rendite')

colnames(reg_vorbereitung_vader)<-c("Sentiment1","Sentiment2", "Sentiment3","Sentiment4","Sentiment5", "Sentiment6","Sentiment7","Sentiment8", "Sentiment9","Rendite")
regression_vader<-lm(Rendite~ I(Sentiment1^9)+I(Sentiment2^8)+I(Sentiment3^7)+I(Sentiment4^6)+I(Sentiment5^5)+I(Sentiment6^4)+I(Sentiment7^3)+I(Sentiment8^2)+I(Sentiment9),data=reg_vorbereitung_vader)
summary(regression_vader)
#----------------------------------------------------------------------------
...
Plot_Sentiment_tweet(clearing_data,"Wochen")
Plot_Sentiment_tweet(clearing_data,"Monat")
datenafinn<-afinn_score_wert(clearing_data,"Wochen")
plot_afinn_score(datenafinn)
