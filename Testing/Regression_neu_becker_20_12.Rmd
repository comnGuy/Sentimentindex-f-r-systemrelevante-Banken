...
afinn <- clearing_data%>% 
    ...
    bing_and_nrc <- bind_rows(clearing_data%>% 
                                ...
                                mutate(method = "Bing et al."),clearing_data %>% inner_join(get_sentiments("loughran"))%>% mutate(method="loughran"),
                              clearing_data %>% 
...
reg_wader_group_wochen<- warp_score %>% group_by(week)%>% summarise(sentiment=sum(Sentiment))%>% mutate(method = "Wader")    
