...
<!--- Beginn --->
\section{Ergebnisse}
Der Kurs einer Aktie setzt sich aus einem Angebot und einer Nachfrage zusammen. Die Nachfrage bzw. das Angebot einer Aktie wird von mehreren Faktoren, wie der aktuelle Stand des Unternehmens, die Art und Weise wie in den Neuigkeitsprotalen berichtet wird u. v. m. beeinflusst. Einer unserer Annahmen ist, dass Twitter ebenfalls ein Einflussfaktor für den Kurs sein kann. Eine ähnliches Vorgehen wurde bereits von Cindy K Soo. \cite{sentiment_housing} bezüglich den Immobilensektor gezeigt. In Ihrem Paper zeigt sie, dass die Lokalnachrichten den Immobilienmarkt um Jahre vorher beeinflussen. Diese Idee wollen wir in dieser Ausarbeitung auf Twitter und dem Finanzmarkt übertragen und beobachten, ob ein Einfluss nachweisbar ist.

Ein Ziel dieser Ausarbeitung ist einen Zusammenhang zwischen dieser Stimmung und der Aktienindizies nachzuweisen. Die Ausgangssituation wird der Finanzbereich und die Aktienindizes aus dem Jahr 2012 sein.
...
Mittels Textmining sollen die Informationen aus den Tweets in einen Sentimentindex umgewandelt werden. Mit diesem Sentimentindex wird versucht mittels einer Regression den Aktienindizes vorherszusagen. Auf der Abbildung \ref{Reg_afinn1} ist eine Regression auf wöchentlichen prozentualen Veränderungen der Aktienindizes dargestellt. Die blauen Punkte stellen die Veränderung des Aktienindizies dar und die Regressionsgrade versucht diese zu erklären. Der erklärende Anteil beträgt $3.84 \%$. 
...
Für den Vergleich der wirtschaftlichen Situation in der EU, wird der \textbf{EURO-STOXX} $50$ herangezogen. Der EURO-STOXX $50$ ist ein Aktienindex, der sich aus den $50$ größten, börsennotierten Unternehmen des Euro-Währungsgebiets zusammensetzt. Der Dow Jones wird für die US-Banken-Tweets genommen.
...
Für die Berechnung der Sentimentindizes werden ausschließlich Tweets betrachtet die mit dem sprachlichen Kürzel \textit{eng} gekennzeichnet sind. Es werden Indizes für Europa und die USA gebaut, über die Tweets der Banken.
...
\item Mit der Funktion \textit{clearing$\_$dataframe} werden die Texte der Tweets in einzelnen Wörter aufgesplittet. Damit besteht ein Tweet aus mehreren Zeilen und eine Zeile jeweils nur aus einem Wort. Anschließend wird das Dataframe von StopWörtern bereinigt, die in der Wörterliste \textit{Stopwords} enthalten sind. Das Wörterbuch steht in R zur Verfügung. Im Kapitel Wörterbuch \ref{Woerterbuch} sind diese beschrieben. Die Tabelle \ref{tab:dataaufbereitung} zeigt einen Ausschnitt des aufbereitenden Dataframes. Es werden in der Grafik \ref{tab:dataaufbereitung} die ersten Tweets der ersten Woche angezeigt.Die Spalte \textit{X} gibt die ID des jeweiligen Tweets an zu dem das Wort gehört. Dieses Dataframe gilt als Grundlage für die nächsten Kapitel. 
...
In diesem Bericht wurden die Wörterbücher \textit{bing}, \textit{afinn}, \textit{loughran} und \textit{nrc} aus dem R-Package \textit{tidytext} und das vaderSentiment \cite{vader} verwendet, siehe. Diese bestehen aus Unigrammen (einzelne Wörter).
...
Die Wörterbücher "bing" und "afinn" liegen können insgesamt um die 70 übereinstimmungen aufweisen, wohingegen "loughren" knapp über 50 vorweisen kann. Das erfolgreichste Wörterbuch ist "vader" mit ca. 120 übereinstimmmungen. 
...
Um das Stemming durchzuführen, wurde das R-Package \textit{corpus} \cite{corpus} verwendet . Die folgenden Varianten des Stemmings wurden durchgeführt. \textit{corpus} unterstützt vom Haus aus 17 unterschiedliche Sprachen, um ein Wortstamm zu bilden. Da der gegebene Datensatz aus englischen Texten besteht, wurde nur die englische Sprache des Standard-Packetes betrachtet. Die folgenden Stemmer und Rechtschreibkorrekturen wurden betrachtet.
...
Die nachstehenden Kapitel basieren auf den Informationen aus der Literatur \cite{textmining}. In diesem Abschnitt werden Methoden angerissen, die Tweets und Sentimentindizes analysieren. Um einen ersten Eindruck von den Tweets zu erhalten, ist es vom Vorteil die am häufigsten vorkommenden positivsten und negativsten Wörter zu visualisieren. Die Visualisierung wird mittels einem Balkendiagramm umgesetzt, zusehen im Anhang \ref{anz:woerter}
...
Zu Beginn wurde das Twitter-Archive bereitgestellt. Aus diesem haben wir versucht sinnvolles Tweets bezüglich der Finanzbranche zu extrahieren. Vor der Extraktion war uns nicht klar, ob genügend Tweets extrahiert werden können, weswegen eine zweite Quelle, die Twitter-API \ref{sub:scanned_data} hinzugezogen wurde. Danach wurde im Kapitel \ref{sub:datenaufbereitung} die Daten aufbereitet und erweitert. Als nächstens haben wir die mehrere Wörterbücher und Stemming-Verfahren verglichen, siehe Kapitel \ref{Woerterbuch}. 
...
Neben weiteren Wörterbüchern, Stemming-Verfahren und besseren Rechtschreibkorrekturen besteht die Möglichkeit anstatt 1-gram (Unigrammen) mit 2-gramme (Bigramme) oder 3-gramme zu arbeiten. Das bedeutet dementsprechend angepasst Wörterbücher zu verwenden. Laut E. Martínez-Cámara aus Ihrem Paper \cite{bigram_ref} ist dieser Ansatz sehr vielversprechend.
...
\fancyhead[L]{A Anzahl der Wörter} 
\addcontentsline{toc}{section}{A Anzahl der Wörter} \label{anz:woerter}
\section*{A Anzahl der Wörter}
 \begin{figure}[H]
\centering
...
\fancyhead[L]{B Berechnung der Sentimentindizes} 
\addcontentsline{toc}{section}{B Berechnung der Sentimentindizes}
\section*{B Berechnung der Sentimentindizes}
...
\fancyhead[L]{C Beschreibung der Programmen und Dateien} 
\addcontentsline{toc}{section}{C Beschreibung der Programmen und Dateien}
\section*{C Beschreibung der Programmen und Dateien}
